...
void init(output *out_ptr, conf *c_ptr) {
	...
	fscanf(fd_conf, "%f %f %f %f %f %f %d %d %d %f %d %f %f %f %d %f %d %f %f %f %f", &(c_ptr->Q1), &(c_ptr->Q2), &(c_ptr->Q3), &(c_ptr->Q1_PAS), &(c_ptr->Q2_PAS), &(c_ptr->Q3_PAS), &(c_ptr->Q1_MAX), &(c_ptr->Q2_MAX), &(c_ptr->Q3_MAX), &(c_ptr->DELTA), &(c_ptr->CP_FLAG), &(c_ptr->LENGTH), &(c_ptr->WEIGTH), &(c_ptr->CONSIGN), &(c_ptr->STOP), &(c_ptr->SATURATION), &(c_ptr->SATURATION_FLAG), &(c_ptr->STATIC_TOLERANCE), &(c_ptr->R), &(c_ptr->BWN), &(c_ptr->MU));
...
void mcc_model(output *out_ptr, conf *c_ptr) {
	out_ptr->speed[0] = (cmd_i[1] * (1 - exp(-(c_ptr->MU) * c_ptr->DELTA / TM)) / c_ptr->MU) + (out_ptr->speed[1] * exp(-(c_ptr->MU) * c_ptr->DELTA / TM));
...
void mcc_real_model(output *out_ptr, conf *c_ptr) {
    float K1 = ((1 - GAMMA) * UN) / (WN * c_ptr->BW0);
    float K2 = J / c_ptr->BWN;
